...
static void Jbdbms_ble::notifyCallback(BLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify)
//this is called when BLE server sents data via notofication
{
    //hexDump((char*)pData, length);
    // bleCollectPacket((char *)pData, length);
    //response
    int header_type = checksumCalc((char *)pData, length);
    if( header_type == 3){
        parseBasicData((char *)pData, length);
    }else if(header_type == 4){
        parseVoltageData((char *)pData, length);
    }
...
boolean Jbdbms_ble::sendCommand(uint8_t *data, uint32_t dataLen)
{
...
Jbdbms_ble::Jbdbms_ble(PubSubClient *_mqttClient, String _BLE_SERVICE_UUID, String _BLE_TX_UUID, String _BLE_RX_UUID, int slave_id, int _cell_series){
...
int Jbdbms_ble::update_data(){
	...
	int cnt = 0;
    int buffer_size;
    uint8_t askBasic[] = {0xDD, 0xA5, 0x03, 0x00, 0xFF, 0xFD, 0x77};
    uint8_t askVoltage[] = {0xDD, 0xA5, 0x04, 0x00, 0xFF, 0xFC, 0x77};

    sendCommand(askBasic, sizeof(askBasic));
    sendCommand(askBasic, sizeof(askVoltage));
    return 0;
}
...
int Jbdbms_ble::set_discharge_mosfet(bool val){
    ...
    uint8_t askMosfet[] = {0xDD, 0x5A, 0xE1, 0x02, 0x00, (uint8_t)mosfet_val, 0xFF, 0x1D-(uint8_t)mosfet_val, 0x77};
    sendCommand(askMosfet, 9);
    return 0;
}
